   ...
   useEffect((()=>{
      ...
      getAllCreatedUsers()
         .then((users)=>{
            ...
            const createData = lastMonth.map((date)=>({date:date,newUsers:0}))
            Object.keys(users).forEach((user)=>{
               const userSignUpDate = new Date(users[user]['createdOn'])
                  createData.map((el)=>{
                     if(el.date===userSignUpDate.getDate() && userSignUpDate.getTime()>now- MONTH_MS){
                        el.newUsers++
                     }
               })
            })
            setData(createData)
            setToday(now.toLocaleDateString('en-US',{month:'long', day:'numeric'}))
            setStartPeriod(start.toLocaleDateString('en-US',{month:'long', day:'numeric'}))
         })
   ...
   return (
      ...
      <Box marginTop={'10'}>
         <Stat textAlign={'center'}>
               ...
               <StatHelpText>{startPeriod} - {today}</StatHelpText>
         </Stat> 
      </Box>

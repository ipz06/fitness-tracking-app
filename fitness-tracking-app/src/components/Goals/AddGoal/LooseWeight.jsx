...
const LooseWeight = ({ open, setOpen }) => {
  ...
  const HandleSubmitGoal = (open, setOpen) => {
    ...
    if (
      Math.abs(targetWeight - currentWeight) / ((targetDate - now) / MONTH_MS) >
      6
    ) {
  ...
  return (
    <>
      ...
      <IconButton
        ...
        size={"md"}
        margin={"5"}
        onClick={() => HandleSubmitGoal(open, setOpen, targetWeight, date)}
      />
    </>
  );
};
